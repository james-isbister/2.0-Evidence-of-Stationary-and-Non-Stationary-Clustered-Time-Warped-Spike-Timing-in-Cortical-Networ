...
The neural coding problem asks how information is encoded through patterns of action potentials in the nervous system \cite{gerstner1997neural, johnson2000neural, stein2005neuronal, gerstner2014neuronal}. Responses following sensory stimulation can be analysed to explore principles of neural coding, with a classic dichotomy between temporal \cite{uzzell2004precision, johansson2004first, gollisch2008rapid, storchi2012comparison, reinagel2002precise, reyes2014laminar} and rate codes \cite{hong2016explicit, chang2017code, kar2019evidence, stringer2019high}. Temporal codes provide the possibility of a fast and efficient code. Whilst precise temporal patterns have been observed \cite{uzzell2004precision, johansson2004first, gollisch2008rapid, storchi2012comparison, reinagel2002precise, reyes2014laminar}, a large proportion of trial-to-trial variability remains unexplained. This high variability leads to common assumption of population firing rate codes, under the premise that variability cancels out across a population \cite{abbott1999effect, moreno2014information, stringer2019high}. Average firing rate codes are abstracted from neural mechanisms (see below), leaving a full understanding of neural coding and variability undetermined.

It is important to better characterise neural codes in order to understand a rich variety of dynamical, structural, homeostatic and cognitive processes. Characterising responses may also uncover response properties involved in behaviour. Due to response variability from trial-to-trial, characterising the neural representation of a single stimulus is a matter of characterising the \textit{distribution} of possible neural responses.

A multi-neuron stimulus response can be expressed as a point in multi-dimensional space, where each dimension represents a spike train metric. Responses from multiple presentations of a stimulus form a sample stimulus-response distribution in this space, which approximates an underlying stimulus-response distribution (Fig. 1a). If two stimuli have a behaviourally relevant difference, downstream neurons must be able to respond differently to samples from the response distribution of either stimulus. This is possible if the response distributions are highly non-overlapping in dimensions that downstream neurons are sensitive to, such as relative spike time differences (\cite{gasparini2006state, branco2010dendritic, branco2011synaptic}; Fig. 1a).

Two phenomena complicate stimulus response distribution characterisation. Firstly, responses of some neurons depend on trial index. That is, response distributions are \textit{non-stationary} due to mechanisms of adaptation \cite{dragoi2002dynamics, ulanovsky2003processing, sharpee2006adaptive, ahissar2000transformation, kheradpezhouh2017response, ahissar2001temporal, khatri2009stimulus, martin2014tactile, barros2019response}. It follows that the region from which stimulus-responses are drawn from at a single point in time is smaller than the region from which samples are drawn from over multiple trials (Fig. 1b).  Adaptation occurs across species and modalities \cite{dragoi2002dynamics, ulanovsky2003processing, sharpee2006adaptive}, and in barrel cortex to repetitive whisker stimulation for both spike count \cite{ahissar2000transformation, ahissar2001temporal, kheradpezhouh2017response, khatri2009stimulus, martin2014tactile, barros2019response} and spike latency \cite{ahissar2000transformation, ahissar2001temporal, kheradpezhouh2017response}. 
...
Secondly, firing correlations between spatially distanced neurons (see \cite{reid2012functional, feldmeyer2013barrel} for review) suggest the possible presence of a \textit{continuous} underlying state covarying neural responses. Response samples on a single trial (when the continuous state has a certain value) would then come from a state-dependent stimulus-response distribution (i.e. a smaller region of response distribution space; Fig. 1c).
If receiving neurons could respond to a state-modulated signal in a state-dependent manner (contrary to common neural coding assumptions \cite{moreno2014information, stringer2019high}), state-dependent encoding would increase the capacity for discriminable stimulus encoding (Fig. 1c). The question is whether correlations between spatially distanced neurons are evidence of a shared state, which receiving neurons could also be modulated by [New Fig?].
% Correlations in the barrel cortex up to a distance of $\sim1.5mm$ (vertically and horizontally) \cite{reyes2015long} suggest it is possible that afferently connected neurons could also be state-modulated and respond in a state-dependent manner. 
Subthreshold membrane potentials are a candidate for such shared state, as they affect action potential initiation \cite{petersen2003interaction} and are correlated across areas \cite{poulet2008internal}.
% State information could alternatively be explicitly encoded in state modulated population activity. 

Estimating state-dependent stimulus-response distributions at a single snapshot in time is made difficult by heterogeneity of non-stationarity and response distribution structures (Supplementary Vid. 1), leading to use of averaging and coarse spike train metrics. As \textit{spike counts} show high trial-to-trial variability, single units cannot reliably encode many different stimuli. Instead, time, trial or population averaged scalar \textit{firing rates} \cite{gerstner2014neuronal} are calculated, with capacity beyond the original single trial discrete spike count, under the assumption that noise cancels out across a population \cite{abbott1999effect, moreno2014information, stringer2019high}.
...
The topographic representation of each single whisker on the snout of a rodent in the somatosensory cortex, the so-called barrel cortex, represents an excellent model to study neuronal processing of defined sensory stimulation \cite{feldmeyer2013barrel}.
The majority of barrel cortex neurons respond 0 or 1 times to single whisker deflections \cite{reyes2014laminar}. First spike patterns allow direct study of the relationship between known entities (e.g. stimulus location, intensity, frequency) and neuronal activity. A recent optogenetics study showed that early visual cortex patterns of single spikes are sufficient to enable visual discrimination \cite{resulaj2018first}, implicating single spike patterns as a candidate for atomic representation. 

First spike times can have millisecond order precision relative to stimulus onset in sensory organs \cite{johansson2004first, uzzell2004precision, gollisch2008rapid}, thalamic areas \cite{reinagel2002precise, storchi2012comparison} and early sensory cortices \cite{reyes2014laminar}, with some evidence of continuum-like encoding relative to onset \cite{gollisch2008rapid, storchi2012comparison} and oscillations \cite{havenith2011synchrony}. Relative differences \cite{izhikevich2006polychronization} observed experimentally \cite{johansson2004first, gollisch2008rapid, reyes2014laminar} have been shown to encode information when trial averaged \cite{gollisch2008rapid} and do not require reference to an onset signal.
...
Spike time differences between two neurons with independent and normal first spike times would have a standard deviation of \(\sigma_{diff} = \sqrt{\sigma_{a}^2 + \sigma_{b}^2}\).
Spike times could covary with unitary gradient under the assumption that relative spike times have a factor-independent mean. Precise relative differences have been reported in the eye \cite{gollisch2008rapid} and barrel cortex \cite{reyes2014laminar}. Response distributions may form multiple clusters, implying stochasticity in the cluster from which representations come (Fig. 1d; \cite{reyes2014laminar}).
...
\subsection*{Overview}
% 
This paper analyses first-spike stimulus-response distributions for pairs of barrel cortex units in response to repeated single whisker deflections. In first-spike stimulus-response distributions for pairs of units, correlated clusters were found above chance-levels. Clustering was necessary for the detection of a large proportion of correlations.
Cluster spike times of both units were stationary for X\%\todo[color=red!40]{Fill} of correlated clusters. X\% of these stationary correlated clusters had non-unitary (different from $45^{\circ}$) correlation angles, suggestive of an internal underlying factor co-varying first spike times and thus \textit{warping} the distribution of (decodable) relative spike time differences from trial-to-trial (Fig. 1d). Estimated factor-conditioned response distributions for these stationary clusters showed a high potential encoding capacity. 

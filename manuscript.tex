...
The neural coding problem addresses the question of how information is encoded by patterns of action potentials in the nervous system \cite{gerstner1997neural, johnson2000neural, stein2005neuronal, gerstner2014neuronal}. Whilst precise temporal spike patterns have been observed \cite{uzzell2004precision, johansson2004first, gollisch2008rapid, storchi2012comparison, reinagel2002precise, reyes2014laminar}, a large proportion of spike time variability remains unexplained. Such variability leads to the common assumption that the brain uses a population code based on spike counts \cite{reyes2015high, lin2015nature} or trial-averaged firing rates \cite{hong2016explicit, chang2017code, kar2019evidence, stringer2019high}. Previous studies have shown that spike count variability is shared across the population in early sensory cortices \cite{scholvinck2015cortical, lin2015nature}. Single cortical neurons can generate highly reliable spike trains to direct stimulation in vitro \cite{mainen1995reliability}. In vivo shared variability is therefore likely to arise from variability in synaptic inputs, reflecting cortical network dynamics \cite{carandini2004amplification}. 

In early visual cortex, trial-to-trial spike count variability is correlated across the population and is modulated by two global factors, which fluctuate the level of excitability from trial-to-trial \cite{lin2015nature}. The first of these is an additive factor, which is explained by the interaction of evoked activity with population-wide spontaneous activity \cite{arieli1996dynamics, scholvinck2015cortical}. The second is a multiplicative factor, which also modulates the trial-to-trial fluctuation in excitability across the population, and has been proposed to be caused by top-down feedback from higher cortices \cite{lin2015nature}. Fluctuations in spontaneous activity have also been shown to gate sensory-evoked responses \cite{luczak2013gating}. 

Fluctuations in trial-to-trial excitability are referred to herein as trial-to-trial fluctuations in continuous cortical state \cite{scholvinck2015cortical}, which may be multi-dimensional and can be separated into short and long-timescale changes. In this paper, long-timescale fluctuations refer to large changes taking place over multiple trials caused by processes such as neural adaptation in response to repetitive stimulation, which occurs across species and modalities \cite{dragoi2002dynamics, ulanovsky2003processing, sharpee2006adaptive}, or changes in the level of anaesthesia. Short-timescale fluctuations refer to random trial-to-trial fluctuations in excitability around the current general level of excitability.

% Other studies have reported only weak spike count correlations in sensory cortices \cite{reyes2015high}.

This study aims to characterise the non-trivial nature of trial-to-trial spike time (rather than spike count) noise correlations, as a component step towards investigating the modulation of spike times by short-timescale fluctuations of a low-dimensional continuous cortical state. 
% which would introduce the possibility of state-dependent encoding and decoding (see below). 
The characterisation controls for longer timescale changes in excitability to estimate how stimulus representations are modulated by trial-to-trial fluctuations in state at a given point in time. 
% This offers a novel prediction of the encoding capacity under a state-dependent spike time encoding regime. 
...
%  low 2-dimensional continuous latent state .  

% It is possible that spike time variability is caused and modulated across a population by such a low-dimensional continuous latent state. 
...
A multi-neuron response can be expressed as a point in space, where each dimension represents a spike train metric. Responses to multiple stimulus presentations form a sample stimulus-response distribution in this space, which can be used to estimate an underlying response distribution from which single-trial responses are `drawn' (Fig. 1a).
...
Neural coding classically considers such response-distributions with respect to fixed-state decoders \cite{averbeck2006neural, moreno2014information, stringer2019high}. `Noise correlations' refer to shared trial-to-trial variability between the responses of multiple neurons over repeated presentations of single stimuli, whilst `signal correlations' concern tuning similarity between neurons. With respect to fixed-state decoders, noise correlations can either increase or decrease encoding capacity \cite{averbeck2006neural}.

% Neural coding is classically interested in determining the dimensions in which response distributions do not overlap \cite{averbeck2006neural}.
...
It is important to consider that if downstream neurons are also modulated by the same population-wide short-timescale fluctuating state, then downstream `decoder' neurons could respond in a state-dependent manner.
...
Characterising noise correlations is complicated by the heterogeneity of sample response distribution structure and long-timescale changes 
...
% Processes of adaptation, which we define as different to continuous latent states underlying trial-to-trial noise correlations, could also 
which, could cause correlative structure in the stimulus-response distribution (Fig. X), and must be controlled for when characterising `adaptation-independent' noise correlations.
...
% towards estimating the potential encoding capacity of a state-dependent spike time encoding and determining the dimensionality of such an underlying latent state. The classical `stimulus-dependency' of noise correlations is left to future studies. 
...
% The spike time noise correlations uncovered in this study 
% offer a new opportunity to study population-wide modulation by low-dimensional latent states and 
% controls for neural adaptation, which occurs in the barrel cortex to repetitive whisker stimulation for both spike count \cite{ahissar2000transformation, ahissar2001temporal, kheradpezhouh2017response, khatri2009stimulus, martin2014tactile, barros2019response} and spike latency \cite{ahissar2000transformation, ahissar2001temporal, kheradpezhouh2017response}.
...
Importantly, many clusters remained correlated with non-unitary gradient after controlling for longer-timescale neural adaptation,
% using time series analysis, 
indicative of short-timescale fluctuating state, which varies the latency of different neurons at a different rate.
% giving evidence of precise non-adaptive noise correlations
% Such non-unitary spike time correlation angles cannot be 
...
This novel finding of `adaptation-independent clustered time-warp spike-timing noise correlations' shows that the distribution of decodable first spike time relative differences for pairs of neurons at a point in time are `warped' from trial-to-trial by some short-timescale fluctuating continuous cortical state.
...
The characterisation of adaptation-independent clustered spike-timing noise correlations is useful under the fixed-state decoder interpretations. More interestingly, these noise correlations open the possibility of state-dependent spike time coding. Factor analysis provided an estimate of state-dependent response distributions of first spike pairs (and relative first spike differences) for each adaptation-controlled correlated cluster, giving a prediction of the capacity of state-dependent encoding, which surpasses previously proposed coding forms. 

Adaptation-independent non-unitary correlated clusters appeared for pairs of neurons spatially separated over multiple barrels and cortical layers. This offers suggestive evidence of a low-dimensional fluctuating state affecting the spike times of spatially distanced neurons, which in turn could modulate downstream neurons and cause them to respond in a state-dependent manner. 
% The number of low-dimensional continuous latent states underlying the correlative structure, however, must be confirmed. 
Estimating whether cluster correlations observed for different pairs of neurons are modulated by a single or low-dimensional shared state was made difficult by low trial overlap between clusters. This paper provides
% in extracting and characterising adaptation-independent correlated clusters provides 
a useful step towards investigating this and shows that consideration of clustering, correlative structure and adaptation, together, are necessary to explain a large proportion of neural variability.
% confirming or rejecting state-dependent spike-time coding. 

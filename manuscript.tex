...
Response distributions may form multiple clusters, implying stochasticity in the cluster from which representations come (Fig. 1d; \cite{reyes2014laminar}).
...
Action potential instantaneity suggests that relative spike codes could superimpose (Fig. 1d). Response distributions may form multiple clusters, implying stochasticity in the cluster from which representations come (Fig. 1d; \cite{reyes2014laminar}).

Using a large data set of spike sorted recordings in anesthetized rat barrel cortex, pairwise first spike response distributions were first observed for repeated presentations of single stimuli. That is for repeated presentations of a single stimulus, the first spike times of two units were plotted against each other for trials on which they both spiked. The reader is strongly recommended to view these plots in Supplementary Vid. 1. Lines, colours and ellipses can be ignored at this stage. Simple observation of these plots shows that clusters of first spike pair responses appear correlated, suggesting that first spike pairs are correlated for pairs of neurons on a subset of trials of a stimulus. 

Moreover, the correlation angle of some of the correlated clusters appears to be different from $45^{\circ}$ (i.e. has a gradient different from 1). Such correlations with `non-unitary gradient' may be indicative that correlations are due to an internal process (i.e. state) rather than variation in the precise timing of stimulus onset. However, this must be considered after checking and controlling for neural adaptive trends.

A novel algorithm extracted the observed correlated clusters and found that they appeared above chance levels. To control for adaptive trends, the cluster spike times of each neuron in the pair were tested for stationarity. The cluster spike times of both units were stationary for X\% of correlated clusters. For the remaining clusters, which had at least one unit with non-stationary cluster spike times, time series analysis was applied to the cluster spike times of the unit to remove the non stationarity (i.e. neural adaptive trend).
...
when the is spike times are correlate. Such first spike response distribution structure can be seen in the top left plots of Supplementary Vid. 1, The the reader is strongly recommended to view. 

Simple observation .  It is clear


It is important for neurophysiology to determine whether correlations between spatially distanced neurons are evidence of a shared continuous state, and whether this shared state also modulates downstream decoding neurons.
...
From observation of first spi

This paper presents a novel algorithm, which characterises the occurrence and nature of correlations between the first spike times of pairs of neurons on subsets of trials. This offers (1) a precise understanding of stimulus responses and correlative variability [Vicente Refs], and (2) as a necessary step towards either rejecting state-dependent coding, or confirming its precise nature. 
...
The characterisation offers a prediction of the encoding resolution of this state-dependent coding form, which would surpass previous unconditioned coding forms. Testing the decoding performance of the uncovered response properties would require explicit knowledge of the underlying state so is left for future studies.
...
first spike responses of two units 
...
analyses first-spike stimulus-response distributions for pairs of barrel cortex units in response to repeated single whisker deflections. The

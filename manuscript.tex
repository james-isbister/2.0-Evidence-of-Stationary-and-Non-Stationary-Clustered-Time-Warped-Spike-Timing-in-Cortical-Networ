...
The neural coding problem addresses the question of how information is encoded by patterns of action potentials in the nervous system \cite{gerstner1997neural, johnson2000neural, stein2005neuronal, gerstner2014neuronal}. Whilst precise temporal spike patterns have been observed \cite{uzzell2004precision, johansson2004first, gollisch2008rapid, storchi2012comparison, reinagel2002precise, reyes2014laminar}, a large proportion of spike time variability remains unexplained. Such variability leads to the common assumption that the brain uses a population code based on spike counts \cite{reyes2015high, lin2015nature} or trial-averaged firing rates \cite{hong2016explicit, chang2017code, kar2019evidence, stringer2019high}. Previous studies have shown that spike count variability is shared across the population in early sensory cortices \cite{scholvinck2015cortical, lin2015nature}. As single cortical neurons can generate highly reliable spike trains to direct stimulation in vitro \cite{mainen1995reliability}, shared variability in vivo is likely to arise from variability in synaptic inputs reflecting cortical network dynamics \cite{carandini2004amplification}.




In early visual cortex, trial-to-trial spike count variability is correlated across the population and is modulated by two global factors, which fluctuate the level of excitability from trial-to-trial \cite{lin2015nature}. The first of these factors is explained by an additive interaction of evoked activity with population-wide spontaneous activity \cite{arieli1996dynamics, scholvinck2015cortical}. The second is a multiplicative factor proposed to be caused by top-down feedback from higher cortices \cite{lin2015nature}. 
Changes in excitability are herein referred to as changes in a continuous cortical state \cite{scholvinck2015cortical}.


% Fluctuations in spontaneous activity also gate sensory-evoked responses \cite{luczak2013gating}. 

 
% which may be multi-dimensional. 
Changes in such cortical state can happen on long or short-timescales, and can be either specific or general across the population (Fig. 0a). In this paper, long-timescale state changes refer to slow changes in neuronal excitability caused by processes such as changes in the level of anaesthesia or neural adaptation \cite{dragoi2002dynamics, ulanovsky2003processing, sharpee2006adaptive}. Such processes are more specific than general across the population. Short-timescale state changes refer to random trial-to-trial fluctuations in excitability around the longer-timescale modulated state. 



% Other studies have reported only weak spike count correlations in sensory cortices \cite{reyes2015high}.

% Study of the modulation of spike times by short-timescale changes in state are limited. 
% Towards investigating whether spike times are modulated by short-timescale fluctuations of a low-dimensional cortical state, this study aims to characterise the nature of spike time noise correlations, whilst controlling for long-timescale changes in excitability.

The paper takes a step towards uncovering the dimensionality of possible population-wide short-timescale state changes, by better characterising pairwise spike time noise correlations, whilst controlling for long-timescale state changes.
% This study aims to characterise the non-trivial nature of trial-to-trial spike time noise correlations, as a component step towards investigating the modulation of spike times by short-timescale fluctuations of a low-dimensional continuous cortical state. 
...
This offers a novel prediction of the encoding capacity under a state-dependent coding regime. 
Characterising spike timing codes is important to understanding a variety of dynamic and plastic processes, including properties of action potential patterns directly involved in cognition. Responses following sensory stimulation can be analysed to explore such principles of neural coding. 
...
When the fluctuating state has a value on a single trial, a response comes from a \textit{state-conditioned} stimulus-response distribution, which would occupy a smaller region of response distribution space than the region sampled over multiple trials (Fig. 1c). State-dependent decoding therefore offers an improved coding capacity.
% , compared to the classical fixed-state decoder interpretation.

Long-timescale changes in excitability make it difficult to accurately characterise the stimulus response distribution from which samples are drawn at a point in time. 
% including noise correlation structure caused by short-timescale fluctuations in excitability. 
Long-timescale changes are also likely to cause correlative structure in the stimulus-response distribution that could be misinterpreted as noise correlations caused by a short-timescale fluctuating state. If the effect of long-timescale changes in excitability can be removed or controlled for, it may be possible to give an estimate of the response distribution on a single trial. This could enable characterisation of noise correlations, which in turn could uncover short-timescale fluctuations in a population-wide state. Removing the effect of long-timescale changes and characterising subsequent adaptation-controlled response distributions, is made difficult by heterogeneity in single unit processes of adaptation and the structure of original sample response distributions. 
...
% Particularly, within cluster spike times vary at different rates w.r.t. an underlying state, such that relative differences vary with res
% by some short-timescale fluctuating continuous cortical state. 
...
% Whilst stimulus-dependency of noise correlations is not investigated in this study, estimates of potential encoding capacity are made by analysis of state-dependent response distribution size. 
...
% Using a large dataset of spike sorted recordings in anesthetized rat barrel cortex, the paper aims to address the following questions:
